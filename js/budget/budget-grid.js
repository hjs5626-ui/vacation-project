...
function onCellClick(row, col) {
  ...
  const type = state.placementType || 'gallery';
  if (type === 'gallery' && !state.placementImage && !state.movingWidget) return;
  ...
  exitPlacementMode();
  showToast(type === 'ledger' ? '가계부가 추가되었습니다!' : 'Widget placed!');

  // Persist widget layout immediately so ledger widgetId survives refresh
  if (state.currentDiary) {
    state.currentDiary.widgets = state.widgets.map((w) => ({
      ...
      cols: w.cols,
      rows: w.rows,
      imageData: w.type === 'ledger' ? null : w.imageData,
    }));
    saveEntries();
  }
}
