...
import { checkPlacement, markCellsOccupied, freeCells, clearCellHighlights } from './grid.js';
...
export function placeWidget(row, col, wCols, wRows, imageData) {
  ...
  let type = state.placementType || 'gallery';
...
export function renderPlacedWidget(w) {
  ...
  if (w.type === 'ledger') el.classList.add('placed-widget-ledger');
  ...
  // Long press to pickup and move (skip when interacting with ledger inputs)
  let pressTimer;
  const cancelPress = () => clearTimeout(pressTimer);

  const startPress = (e) => {
    if (e.target.closest('input, button, .ledger-sheet')) return;
...
export function pickupWidget(w) {
  ...
  state.movingWidget = w;
  state.placementType = w.type || 'gallery';
...
export function restoreWidget(w) {
  const widgetId = w.id || `w_${state.widgetIdCounter++}`;
  // Keep counter ahead of restored numeric suffixes
  const match = String(widgetId).match(/w_(\d+)/);
  if (match) {
    state.widgetIdCounter = Math.max(state.widgetIdCounter, Number(match[1]) + 1);
  }
...
export function enterPlacementMode(size, imageData) {
  ...
  state.placementImage = imageData || null;
  if (!state.placementType) state.placementType = imageData ? 'gallery' : 'ledger';
