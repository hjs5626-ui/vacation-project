...
function applyGroupTabColor(tab, group) {
  if (!group?.color) return;

  const accent = getGroupColor(group);
  tab.classList.add('todo-tab--group-color');
  applyAccentColorStyles(tab, accent);
  tab.style.setProperty('--group-tab-color', accent);
  tab.style.setProperty('--group-tab-soft', hexToRgba(accent, 0.16));
  tab.style.setProperty('--group-tab-border', hexToRgba(accent, 0.42));
  tab.style.setProperty('--group-tab-glow', hexToRgba(accent, 0.28));
}
...
function findTodoWidgetById(widgetId) {
  if (!widgetId) return null;

  const inGrid = state.widgets.find((w) => w.id === widgetId);
...
function getTodoModalEls() {
  const overlay = document.getElementById('todo-modal-overlay') || dom.todoModalOverlay;
  const modal = document.getElementById('todo-modal') || dom.todoModal;
  const title = document.getElementById('todo-modal-title') || dom.todoModalTitle;
  const body = document.getElementById('todo-modal-body') || dom.todoDetailBody;
  const closeBtn = document.getElementById('todo-modal-close') || dom.todoModalClose;

  if (overlay) dom.todoModalOverlay = overlay;
  if (modal) dom.todoModal = modal;
  if (title) dom.todoModalTitle = title;
  if (body) dom.todoDetailBody = body;
  if (closeBtn) dom.todoModalClose = closeBtn;

  return { overlay, modal, title, body, closeBtn };
}
...
function getActiveTodoWidget() {
  ...
  return findTodoWidgetById(activeTodoWidgetId);
}
...
function formatTodoCreatedAt(todo) {
  ...
  const diffDays = Math.round((startOfThat - startOfToday) / 86400000);
...
function buildTodoMain(todo, titleClass) {
  ...
  const metaText = formatTodoCreatedAt(todo);
  if (metaText) {
...
function setActiveTab(widgetId, tabId) {
  const w = findTodoWidgetById(widgetId);
  if (!w) return;
...
function openGroupManageMenu(anchorEl, groupId) {
  ...
  colorBtn.dataset.action = 'menu-change-group-color';
...
function handleGroupManageMenuAction(action) {
  ...
  const w = findTodoWidgetById(groupManageContext.widgetId);
  if (!w) {
    closeGroupManageMenu();
    return;
  }
  ...
  } else if (action === 'menu-change-group-color') {
    const group = findGroup(w, groupId);
    if (group) changeTodoGroupColor(w, group);
...
async function changeTodoGroupColor(w, group) {
  const color = await openColorPickerDialog({
    title: '색상 변경',
  ...
  group.color = normalizeHexColor(color, DEFAULT_CATEGORY_COLOR);
  syncTodoWidgetView(w.id);
  persistTodoLayout();
}
...
function buildWidgetTaskItem(todo, accentColor) {
  ...
  item.appendChild(buildTodoMain(todo, 'todo-task-title'));
  return item;
}
...
export function bindTodoWidgetEvents(el) {
  if (!el || el.dataset.todoBound) return;
  el.dataset.todoBound = '1';
  ...
  el.addEventListener('click', (e) => {
    ...
    if (e.target.closest('.todo-widget-resize, .todo-widget-delete, .widget-delete, .todo-tab-add, .widget-thumbnail-edit')) {
      return;
    }
...
function consumeSecondClick(key, windowMs = 800) {
  const now = Date.now();
  if (lastNameClickKey === key && now - lastNameClickAt <= windowMs) {
    lastNameClickKey = null;
    lastNameClickAt = 0;
    return true;
  }
  lastNameClickKey = key;
  lastNameClickAt = now;
  return false;
}
...
export function openTodoDetailPanel(widgetId) {
  ...
  const { overlay, title } = getTodoModalEls();
  if (!overlay) return;

  activeTodoWidgetId = widgetId;

  if (title) title.textContent = 'To-Do';
...
export function closeTodoDetailPanel() {
  ...
  const { overlay, body } = getTodoModalEls();
  if (overlay) {
...
function buildTaskItem(todo, accentColor) {
  ...
  const main = buildTodoMain(todo, 'todo-detail-task-title');
...
function buildCategoryCard(w, group, category, showGroupLabel) {
  ...
  const colorBtn = document.createElement('button');
  colorBtn.type = 'button';
  colorBtn.className = 'todo-category-color-btn';
  colorBtn.dataset.action = 'change-category-color';
  colorBtn.dataset.groupId = group.id;
  colorBtn.dataset.categoryId = category.id;
...
export function renderTodoDetailPanel() {
  ...
  const w = getActiveTodoWidget();
  const { body } = getTodoModalEls();
  if (!w || !body) return;

  ensureTodoWidgetData(w);
  body.replaceChildren();

  body.appendChild(buildDetailGroupTabs(w));
  ...
  if (w.groups.length > 0) {
    body.appendChild(buildDetailFooterActions());
  }
}
...
function isCategoryGroupReorderTarget(target, card) {
  ...
  if (target.closest('.todo-category-toggle, .todo-category-add-btn, .todo-category-name, .todo-category-color-btn')) return false;
...
export function bindTodoDetailEvents() {
  const { overlay, modal, body, closeBtn } = getTodoModalEls();
  if (!overlay || overlay.dataset.bound) return;
  overlay.dataset.bound = '1';

  closeBtn?.addEventListener('click', closeTodoDetailPanel);

  overlay.addEventListener('click', (e) => {
    if (e.target === overlay) closeTodoDetailPanel();
  });
  ...
  body.addEventListener('click', (e) => {
    ...
    if (actionEl) {
      ...
      if (action === 'change-category-color') {
        const category = findCategory(w, groupId, categoryId);
        if (category) changeTodoCategoryColor(w, groupId, category);
        return;
      }
    ...
    if (detailTab) {
      ...
      if (tabId === ALL_TAB) {
        if (w.activeTab !== ALL_TAB) setActiveTab(w.id, ALL_TAB);
        return;
      }

      if (w.activeTab === tabId) {
        openGroupManageMenu(detailTab, tabId);
        return;
      }
    ...
    if (nameEl) {
      ...
      if (!header) return;
      const key = `cat:${header.dataset.groupId}:${header.dataset.categoryId}`;
      if (consumeSecondClick(key)) {
    ...
    if (header) {
      if (e.target.closest('.todo-category-toggle, .todo-category-add-btn, .todo-category-drag-handle, .todo-category-color-btn, .todo-category-name')) return;
  ...
  body.addEventListener('dblclick', (e) => {
    ...
    if (detailTab) {
      ...
      const anchor = body.querySelector(`.todo-detail-tab[data-tab-id="${tabId}"]`);
...
export function bindTodoComposeSheetEvents() {
  if (!dom.todoComposeOverlay || !dom.todoComposeCancel || !dom.todoComposeSubmit) return;
